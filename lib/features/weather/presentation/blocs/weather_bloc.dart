...
class WeatherBloc extends Bloc<WeatherEvent, WeatherState> {
  ...
  WeatherBloc(this._getCurrentWeather, this._getWeatherForecast)
	: super(const WeatherState.initial()) {
	on<WeatherEvent>((event, emit) async {
	  await event.map(
		fetchWeather: (e) => _fetchWeather(e, emit),
	  );
	});
  }
  ...
  Future<void> _fetchWeather(
	...
	final currentWeatherResult = await _getCurrentWeather.execute(
	  lat: event.lat,
	  lon: event.lon,
	);

	final forecastResult = await _getWeatherForecast.execute(
	  lat: event.lat,
	  lon: event.lon,
	);
